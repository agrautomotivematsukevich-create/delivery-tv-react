...
export const api = {
  ...
  fetchTasks: async (
    mode: "get_operator_tasks" | "get_stats"
  ): Promise<Task[]> => {
    try {
  ...
  fetchFullPlan: async (dateStr: string): Promise<PlanRow[]> => {
    try {
      const res = await fetch(
        buildUrl({ mode: "get_full_plan", date: dateStr })
      );
      const data = await res.json();
      return Array.isArray(data) ? data : [];
    } catch (e) {
  ...
  fetchDowntimeReasons: async (date: string): Promise<DowntimeReason[]> => {
    try {
      const res = await fetch(buildUrl({ mode: "get_downtime_reasons", date }));
      const data = await res.json();
      return Array.isArray(data) ? data : [];
    } catch (e) {
      console.error(e);
