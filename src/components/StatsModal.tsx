                       ...
                       {tasks.filter(t => t.status === 'DONE').map(task => (
                          <div key={task.id} className="flex justify-between items-center p-4 bg-white/5 rounded-xl hover:bg-white/10 transition-colors">
                             <div className="flex flex-col gap-1">
                               ...
                               <span className="text-[10px] text-white/30 uppercase tracking-wider">{t.list_done}</span>
                             </div>
                             <div className="text-right">
                               <div className="font-mono text-sm text-white/40 tabular-nums">{task.end_time || task.time}</div>
                               {task.duration && (
                                  <div className="text-xs font-bold text-accent-green tabular-nums mt-1 bg-accent-green/10 px-2 py-0.5 rounded border border-accent-green/20 inline-block">
                                    {task.duration}
                                  </div>
                               )}
                             </div>
