...
import { Camera, Lock, CheckCircle, Upload, ClipboardCheck } from 'lucide-react';
...
const ActionModal: React.FC<ActionModalProps> = ({ action, user, t, onClose, onSuccess }) => {
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      reader.onload = (evt) => {
        ...
        img.onload = () => {
          ...
          let suffix = "";
          if (isStart) {
             suffix = currentPhotoTarget.current === 1 ? "_General" : "_Seal";
          } else {
             suffix = currentPhotoTarget.current === 1 ? "_Empty" : "_Inspect";
          }
  ...
  const isFormValid = () => {
    if (isStart) {
      ...
      // Finish requires both Empty photo and Inspection photo
      if (!deferUpload) {
        return !!photo1 && !!photo2;
      }
      return true;
  ...
  const handleSubmit = async () => {
    ...
    let urlGen = "", urlSeal = "", urlEmpty = "", urlInspect = "";

    if (!deferUpload) {
      if (photo1) {
        if (photo2) setUploadStatus(`${t.msg_uploading} (1/2)`);
        else setUploadStatus(`${t.msg_uploading}`);
        
        const u1 = await api.uploadPhoto(photo1.data, photo1.mime, photo1.name);
        
        if (isStart) urlGen = u1;
        else urlEmpty = u1;
      }
      
      if (photo2) {
        setUploadStatus(`${t.msg_uploading} (2/2)`);
        const u2 = await api.uploadPhoto(photo2.data, photo2.mime, photo2.name);
        
        if (isStart) urlSeal = u2;
        else urlInspect = u2;
      }
    }
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-in fade-in zoom-in duration-200">
      <div className="bg-[#0F0F12] border border-white/10 p-8 rounded-3xl w-full max-w-[480px] flex flex-col gap-6 shadow-2xl">
        ...
        <div className="space-y-3">
           ...
           >
             {photo2 ? <CheckCircle className="text-accent-green w-8 h-8" /> : (isStart ? <Lock className="text-white/50 w-8 h-8" /> : <ClipboardCheck className="text-white/50 w-8 h-8" />)}
             <span className="font-semibold text-white/80">{isStart ? t.lbl_photo2 : t.lbl_photo_inspection}</span>
           </div>
