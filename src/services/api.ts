...
const SECRET_KEY = "WH_SECURE_KEY_2024";
...
export const parseDashboardData = (text: string): DashboardData | null => {
  try {
    if (!text || text.includes("DOCTYPE") || text.includes("Error")) return null;
...
export const api = {
  ...
  updateUserName: async (row: number, newName: string): Promise<boolean> => {
    try {
       await fetch(`${SCRIPT_URL}?mode=update_user_name&row=${row}&name=${encodeURIComponent(newName)}`);
       return true;
    } catch (e) { return false; }
  },
  ...
  sendMessage: async (user: string, text: string): Promise<boolean> => {
    try {
      ...
      await fetch(`${SCRIPT_URL}?mode=send_message&user=${encodeURIComponent(user)}&text=${encodeURIComponent(encrypted)}`);
      return true;
    } catch (e) { return false; }
  },

  clearMessages: async (): Promise<boolean> => {
    try {
      await fetch(`${SCRIPT_URL}?mode=clear_messages`);
      return true;
    } catch (e) { return false; }
  }
};