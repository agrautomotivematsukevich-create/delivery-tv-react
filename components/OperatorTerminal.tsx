                ...
                <div key={task.id} ref={isFirstActive ? activeRef : undefined}
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 ml-auto shrink-0">
                      ...
                      <button
                        onClick={() => { vibrate(30); onTaskAction(task, isWait ? 'start' : 'finish'); }}
                        className={`h-10 px-5 rounded-xl font-bold text-sm tracking-wide transition-all active:scale-95 flex items-center gap-2 ${
                          isWait ? 'bg-accent-blue text-white hover:bg-accent-blue/80' : 'bg-accent-green text-black hover:bg-accent-green/80'
                        }`}>
                        {isWait ? <><Play size={13} fill="currentColor" /> {t.btn_start}</> : <><Check size={15} /> {t.btn_finish}</>}
                      </button>
                    </div>
                  </div>
