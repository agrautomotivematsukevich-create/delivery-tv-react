...
const OperatorTerminal: React.FC<OperatorTerminalProps> = ({ onClose, onTaskAction, t, currentUser }) => {
  ...
  // Для предотвращения лишних ререндеров – храним предыдущие данные
  ...
  const fetchQueue = useCallback(async () => {
    try {
      ...
      // Сравниваем с предыдущими данными – обновляем состояние только если есть изменения
  ...
  // Обёртка для onTaskAction – сохраняет/удаляет activeTaskId в localStorage
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-lg font-semibold text-white">
                    {calculateDuration(myActiveTask.start_time)}
                  </div>
                </div>
              </div>
      ...
      <div className="bg-[#0A0A0C] w-full md:w-[95%] max-w-[800px] h-[95vh] md:h-[90vh] rounded-t-3xl md:rounded-[2.5rem] border border-white/10 flex flex-col shadow-2xl overflow-hidden relative">
        ...
        <div className="px-6 py-4 bg-white/5 border-b border-white/10">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-white/60">
              Активных (система): <span className="font-bold text-accent-yellow">{allActiveTasks.length}</span>
            </div>
          </div>
        </div>
