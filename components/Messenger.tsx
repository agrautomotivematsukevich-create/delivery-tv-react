...
const Messenger: React.FC<MessengerProps> = ({ onClose, user, t }) => {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-[100] w-full max-w-sm bg-[#0F0F12] border-l border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col animate-in slide-in-from-right duration-300 backdrop-blur-3xl">
       ...
       {/* Chat Area */}
       <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-gradient-to-b from-black/20 to-transparent">
          {messages.length === 0 && (
             <div className="text-center text-white/20 mt-10 text-sm">No messages yet. Start chatting!</div>
          )}
          {messages.map((m) => {
