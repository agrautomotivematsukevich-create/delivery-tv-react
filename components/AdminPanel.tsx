...
import { ShieldAlert, UserCheck, XCircle, RefreshCcw, CheckCircle } from 'lucide-react';
      ...
      <div className="w-full max-w-3xl flex flex-col gap-6 relative mt-10">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center p-12 bg-white/5 rounded-2xl border border-white/10">
            ...
            <p className="text-white/50 text-xl font-bold">Нет новых заявкок</p>
          </div>
        ) : (
            ...
            {users.map(u => (
              <div key={u.login} className="flex flex-col md:flex-row items-center justify-between p-5 bg-white/5 border border-white/10 rounded-2xl gap-4">
                <div className="flex-1 min-w-0 flex flex-col gap-1">
                  ...
                  <div className="text-xs font-bold text-accent-blue uppercase mt-1">{u.role}</div>
                </div>
                <button
                  onClick={() => handleApprove(u.login)}
                  disabled={approving === u.login}
                  className="w-full md:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 border border-emerald-500/30 rounded-xl font-bold transition-all disabled:opacity-50"
                >
                  {approving === u.login ? (
